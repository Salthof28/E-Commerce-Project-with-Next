...
interface UsersHandleCliensProps {
    initialData: initialData
}

export default function UsersHandleCliens ({ initialData }: UsersHandleCliensProps) {
    const { session } = useAuthAdmin();
    const [users, setUsers] = useState<Users[]>(initialData.dataUsers);
    ...
    const [allUsers, setAllUsers] = useState<Users[]>(initialData.dataUsers);
    ...
    const fetchDataUsers = async () => {
        try{
            ...
            setTotalPage(Math.ceil(totalUsers/initialData.usersPerPage));
            setAllUsers(data);
    ...
    const handlePageChange = (page: number) => {
        setCurrentPage(page);
        const start = (currentPage - 1) * initialData.usersPerPage;
        const end = start + initialData.usersPerPage;
        if(search){
            const filterUsers: Users[] = allUsers.filter(users => users.name.toLowerCase().includes(search.toLowerCase()));
            setTotalPage(Math.ceil(filterUsers.length/initialData.usersPerPage));
            setUsers(filterUsers.slice(start, end))
        }
        else {
            setTotalPage(Math.ceil(allUsers.length/initialData.usersPerPage));
