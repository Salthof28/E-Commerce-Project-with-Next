...
export default function ProductForm ({ product, onCancel, titleForm, category, onSubmit}: ProductFormProps) {
    ...
    return(
        <form onSubmit={(e) => {e.preventDefault(); onSubmit(formData);}} className="flex flex-col bg-[#32190c6e] p-[1rem] rounded-[1rem] gap-[0.5rem] 2xl:w-[100%]">
            ...
            <section className="flex flex-col items-center gap-[1rem] p-[1rem] justify-center border border-gray-300/20 rounded-[1rem]">
                ...
                <section className="flex flex-col items-center gap-[0.5rem]">
                    ...
                    <div className="flex flex-col md:flex-row gap-[0.5rem] items-center">
                        ...
                        <button data-testid='btnAddImage' disabled={newImage.trim() === ""} onClick={handleAddImage} type="button" className={`${newImage.trim() === "" ? 'bg-gray-500 cursor-not-allowed' : 'bg-emerald-500 hover:bg-emerald-700 hover:text-white active:scale-95 duration-200'} p-[0.4rem] rounded-[0.4rem] max-md:w-[6rem] text-[0.6rem] md:text-[0.8rem]`}>Add New Image</button>
                    </div>
                </section>
                {/* section List Images */}
                ...
                <section className="flex flex-row gap-[0.5rem] justify-center flex-wrap">
                    ...
                    <div key={index} className="flex flex-col items-center border border-gray-300/20 p-[0.5rem] rounded-[1rem] gap-[0.4rem] bg-[#040414f5]">
                        <img data-testid='listImg' className="border w-[6rem] h-[6rem] md:w-[4rem] md:h-[4rem] rounded-[50%]" src={image}></img>
                        <label>Image url:</label>
                        <input data-testid="inptImageList" value={image} data-index={index} onChange={handleChange} type="text" name="image" className="bg-[rgb(8,5,3)] rounded-[0.5rem] border border-gray-300/20 p-[0.3rem] text-[0.9rem]"/>
                        <div className="flex flex-row gap-[0.4rem]">
                            <button data-testid='btnDeleteImage' onClick={() => handleDeleteImage(index)} type="button" className="bg-red-600 p-[0.2rem] rounded-[0.4rem] hover:bg-red-800 hover:text-white active:scale-95 duration-200 text-[0.8rem]">Delete Image</button>
                        </div>
                    </div>
                    ))}
                </section>
            ...
            <label>Title:</label>
            <input data-testid="inptTitle" value={formData.title} onChange={handleChange} type="text" name="title" className="bg-[rgb(8,5,3)] rounded-[0.5rem] border border-gray-300/20 p-[0.3rem]"/>
            <label>Price:</label>
            <input data-testid="inptPrice" value={formData.price} onChange={handleChange} type="text" name="price" className="bg-[rgb(8,5,3)] rounded-[0.5rem] border border-gray-300/20 p-[0.3rem]"/>
            <label>Description:</label> 
            <textarea data-testid="inptDescription" value={formData.description} onChange={handleChange} name="description" className="bg-[rgb(8,5,3)] rounded-[0.5rem] border border-gray-300/20 p-[0.3rem]" rows={5} />
            <label>Category:</label>
            <select data-testid='categorySelect' value={formData?.categoryId} onChange={handleChange} name="category" className="bg-[rgb(8,5,3)] rounded-[0.5rem] border border-gray-300/20 p-[0.3rem]" >
